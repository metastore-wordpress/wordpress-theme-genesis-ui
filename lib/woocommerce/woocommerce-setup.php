 ...
 * Genesis UI.
 *
 * This file adds the required WooCommerce setup functions to the Genesis UI Theme.
 *
 * @package Genesis UI
 * @author  StudioPress
 * @license GPL-2.0+
...
add_action( 'wp_enqueue_scripts', 'genesis_ui_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function genesis_ui_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'genesis_ui_woocommerce_breakpoint' );
/**
 ...
 */
function genesis_ui_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'genesis_ui_default_products_per_page' );
/**
 ...
 */
function genesis_ui_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'genesis_ui_woocommerce_pagination' );
/**
 ...
 */
function genesis_ui_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'genesis-ui' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'genesis-ui' ) );

	return $args;

}

add_action( 'after_switch_theme', 'genesis_ui_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function genesis_ui_woocommerce_image_dimensions_after_theme_setup() {
	...
	genesis_ui_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'genesis_ui_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function genesis_ui_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function genesis_ui_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'genesis_ui_gallery_image_thumbnail' );
/**
 ...
 */
function genesis_ui_gallery_image_thumbnail( $size ) {
